...
import { Home, Briefcase, MessageSquare, FileText, Route as RouteIcon, Command, Plus } from "lucide-react";
...
const NAV: Array<{ to: string; label: string; icon: typeof Home; color: string; exact?: boolean }> = [
  ...
  { to: "/app/interviews/console", label: "Live interview", icon: MessageSquare, color: "text-sky" },
  { to: "/app/reports/alex-morgan", label: "Evidence report", icon: FileText, color: "text-emerald" },
  { to: "/app/journey/alex-morgan", label: "Candidate journey", icon: RouteIcon, color: "text-purple" },
];
        ...
        <aside className="sticky top-0 hidden h-screen w-[236px] shrink-0 flex-col border-r border-hairline/60 bg-sidebar px-3 py-5 md:flex">
          ...
          <div className="mt-6 px-2">
            <div className="group flex h-9 items-center gap-2 rounded-xl border border-hairline/70 bg-background/80 px-2.5 text-xs text-muted-foreground shadow-[0_1px_0_0_rgb(255_255_255/0.6)_inset,0_1px_2px_-1px_rgb(30_30_60/0.04)] transition-all focus-within:border-brand/40 focus-within:ai-glow">
              ...
              <kbd className="mono-label rounded-md border border-hairline bg-surface px-1.5 py-0.5 text-[9px] text-muted-foreground">⌘K</kbd>
            </div>
          </div>
          ...
          <nav className="mt-6 space-y-1.5">
            {NAV.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon strokeWidth={active ? 2 : 1.6} className={cn("h-[17px] w-[17px] transition-colors", active ? "text-brand" : cn(item.color, "opacity-75 group-hover:opacity-100"))} />
                  {item.label}
                </Link>
              );
            })}
