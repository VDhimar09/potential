...
import {
  COMPETENCY_REPORTS as COMPETENCIES,
  REPORT_TIMELINE as TIMELINE,
...
export const Route = createFileRoute("/app/reports/$id")({
  ...
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: "An evidence-based report. No scores. No rankings. Humans decide.",
      },
    ],
  }),
});
      ...
      <div className="relative mx-auto max-w-[920px] px-10 pb-32 pt-10">
        ...
        <header className="mt-14 rise-in">
          ...
          <div className="mt-3 text-[15px] text-muted-foreground">
            {ALEX_MORGAN.role} · Interview conducted 12 April, 2027
          </div>
        </header>
        ...
        <section className="mt-16">
          ...
          <p className="mt-5 max-w-2xl font-display text-[30px] leading-[1.25] text-foreground md:text-[36px]">
            Alex leads through evidence rather than authority. Strong signals across systems
            thinking, ownership and written communication.
            <span className="text-muted-foreground">
              {" "}
              Their behaviour under direct conflict has not yet been observed.
            </span>
          </p>
        ...
        <section className="mt-20">
          ...
          <div className="mt-6 space-y-4">
            {COMPETENCIES.map((c) => (
              <div
                key={c.name}
                className="rounded-[24px] border border-hairline/60 bg-background/60 p-8 backdrop-blur-xl transition-shadow hover:shadow-soft"
              >
                ...
                {c.evidence.length > 0 && (
                  <div className="mt-6 space-y-3">
                    {c.evidence.map((e, i) => (
                      <blockquote
                        key={i}
                        className="group flex gap-3 rounded-xl border border-hairline/40 bg-surface/50 p-4 transition-colors hover:bg-surface"
                      >
          ...
          <ol className="relative mt-8 space-y-5 border-l border-hairline/70 pl-8">
            {TIMELINE.map((t) => (
              <li key={t.t} className="relative">
                ...
                />
                <div className="text-[11px] font-medium tabular-nums text-muted-foreground/70">
                  {t.t}
                </div>
          ...
          <div className="rounded-[24px] border border-hairline/60 bg-background/60 p-8 backdrop-blur-xl">
            ...
            <ul className="mt-5 space-y-3 text-[14px] text-foreground">
              <li className="flex items-start gap-2.5">
                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-amber-500 glow-pulse" />{" "}
          ...
          <div className="relative overflow-hidden rounded-[24px] border border-secondary/30 bg-secondary/[0.06] p-8">
            ...
            <div className="relative">
              ...
              <p className="mt-5 text-[15px] leading-relaxed text-foreground">
                A 30-minute conversation focused on a project where Alex disagreed with a peer they
                respected. The intent is to hear how they navigate — not to test them.
              </p>
              <Link
...
function ConfidenceBadge({ confidence }: { confidence: Confidence }) {
  const map = {
    High: {
      dot: "bg-emerald",
      text: "text-emerald",
      glow: "shadow-[0_0_18px_oklch(0.68_0.12_168/0.5)]",
    },
    Partial: {
      dot: "bg-amber-500",
      text: "text-amber-600 dark:text-amber-400",
      glow: "shadow-[0_0_14px_oklch(0.78_0.14_80/0.4)]",
    },
